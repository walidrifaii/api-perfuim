...
let CloudinaryService = class CloudinaryService {
    ...
    uploadBuffer(buffer, filename, options = {}) {
        const opts = Object.assign({ folder: 'products', resource_type: 'image', use_filename: true, unique_filename: true, overwrite: false, filename_override: filename }, options);
        return new Promise((resolve, reject) => {
            const stream = this.cld.uploader.upload_stream(opts, (err, res) => {
                if (err)
                    return reject(err);
                resolve(res);
            });
